...
module Class : sig
  (* DWARF-4 specification section 7.5.4, page 147. "Each class is a set of
     forms which have related representations and which are given a common
     interpretation according to the attribute in which the form is used." *)
...
module Form : sig
  ...
  (** We omit the "DW_FORM_" prefix. "Indirect" is not currently supported. *)
...
module Attribute_specification : sig
  (* Attribute specifications: pairs of attributes and forms (DWARF-4
     specification section 7.5.3, page 146). *)

  type 'form t

  val create :
    'dwarf_classes Attribute.t -> ('dwarf_classes, 'form) Form.t -> 'form t
