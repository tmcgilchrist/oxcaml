...
module Form = struct
  ...
  let name (type dwarf_class form) (t : (dwarf_class, form) t) =
    let name =
      match t with
      | Addr -> "addr"
      | Block -> "block"
  ...
  let encode (type dwarf_class form) (t : (dwarf_class, form) t) =
    let code =
...
module Attribute = struct
  ...
  let encode t =
    Dwarf_value.uleb128 ~comment:(name t)
      (Uint64.of_nonnegative_int_exn (code t))
  ...
  let seal (t : _ t) : Sealed.t = { code = code t; name = name t }
end

module Attribute_specification = struct
  type 'form t =
    | T : 'dwarf_classes Attribute.t * ('dwarf_classes, 'form) Form.t -> 'form t
    ...
    include Identifiable.Make (struct
      ...
      let compare t1 t2 =
        ...
        | T1 (T (attr1, form1)), T1 (T (attr2, form2)) ->
          let c =
            Stdlib.compare (Attribute.encode attr1) (Attribute.encode attr2)
          in
          if c <> 0
          then c
          else Stdlib.compare (Form.encode form1) (Form.encode form2)
      ...
      let print ppf (T1 (T (attr, form))) =
        Format.fprintf ppf "%s : %s" (Attribute.name attr) (Form.name form)
